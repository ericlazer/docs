...
They are also central to why Nexio can support undercollateralized institutional credit. An overcollateralized structure can reduce one kind of loss risk by relying on excess pledged assets and liquidation mechanics. An undercollateralized structure relies on a different protection stack: borrower-specific underwriting, documented legal obligations, information rights, covenant testing, ongoing monitoring, and enforceable remedies. Neither model is automatically "safe" or "unsafe" in every context. The relevant question is what protections exist, how they are monitored, and whether they can be enforced when conditions deteriorate.

### **Why Nexio Uses Covenant-Based Credit Controls**

Nexio is not designed around a blanket assumption that every BTC loan should be overcollateralized at all times. Many institutional borrowers use BTC financing for market-making, arbitrage, treasury, or hedging activity where full overcollateralization is capital-inefficient and economically misaligned with the borrower's operating model.
